...
int main(void) {
    ...
    InitWindow(ANCHO + HUD_ANCHO, ALTO + 50, "Tetris con Raylib");
    SetTargetFPS(60);
    srand(time(0));
    ...
    Pieza actual, siguiente; //variables
    ...
    while (!WindowShouldClose()) {
        // Entrada del usuario
        Pieza copia = actual;
        if (IsKeyPressed(KEY_DOWN)) actual.orig.y++;
        if (IsKeyPressed(KEY_LEFT)) actual.orig.x--;
        if (IsKeyPressed(KEY_RIGHT)) actual.orig.x++;
...
void DibujarCuadrado(int x, int y, Color color) {
    ...
    DrawRectangleLines(x * TAM, y * TAM, TAM, TAM, BLACK);
}
...
bool VerificarColision(const Tablero tablero, const Pieza *pieza) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (x < 0 || x >= COLUMNAS || tablero[x][y] != 0)
            return true;
    }
    return false;
}
